...
async function createSyncRun(
  ...
  const { data, error } = await adminClient
    ...
    .insert({
      ...
      sync_type: 'manual_account_metrics',
      status: 'running',
...
async function buildNoRowsDiagnostic(params: {
  connection: SocialConnectionRecord;
  expectedDatasource: string;
  ...
  try {
    ...
    console.error('[social-sync-connection] Windsor no rows diagnostic failed:', {
...
Deno.serve(async (request) => {
  ...
  let loadedConnection: SocialConnectionRecord | null = null;
  let adminClient: Awaited<ReturnType<typeof requireAuthenticatedUser>>['adminClient'] | null = null;
  ...
  try {
    ...
    const connection = await loadConnection(adminClient, { profileId, connectionId });
    ...
    const datasource = connection.provider_datasource || platformConfig.windsorDatasource;
    const { today, yesterday } = getDailySyncWindow();

    syncRunId = await createSyncRun(adminClient, {
    ...
    const connectorAccountResolution = await resolveConnectorAccountId(adminClient, {
      connection,
    ...
    if (scopedSnapshotRows.length === 0 && scopedDailyMetricRows.length === 0) {
      ...
      await finishSyncRun(adminClient, syncRunId, {
        status: 'partial',
      ...
      await updateConnectionSyncStatus(adminClient, {
        connectionId: connection.id,
        success: false,
    ...
    if (rowsToUpsert.length > 0) {
      const { error } = await adminClient.from('social_account_metrics').upsert(rowsToUpsert, {
    ...
    console.error('[social-sync-connection] error:', {
      code: syncError.code,
      message: sanitizedErrorMessage,
      profileId: loadedConnection?.profile_id ?? null,
      connectionId: loadedConnection?.id ?? null,
    });
    ...
    if (adminClient && syncRunId) {
      await finishSyncRun(adminClient, syncRunId, {
        ...
        console.error(
          '[social-sync-connection] failed to update sync run:',
          sanitizeErrorMessage(syncRunError)
        );
      });
    }

    if (adminClient && loadedConnection) {
      await updateConnectionSyncStatus(adminClient, {
        ...
        console.error(
          '[social-sync-connection] failed to update connection sync status:',
