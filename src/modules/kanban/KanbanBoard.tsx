...
export const KanbanBoard = () => {
  ...
  const canManageKanban = canAccess('kanban');
  ...
  const handleAddColumn = async () => {
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite editar o Kanban.');
      return;
    }
  ...
  const persistColumnOrder = async (updatedColumns: KanbanColumn[]) => {
    if (!supabase || !canManageKanban) return;
  ...
  const handleRenameColumn = async (columnId: string, newName: string) => {
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite editar colunas.');
      return;
    }
  ...
  const handleDeleteColumn = async (columnId: string) => {
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite excluir colunas.');
      return;
    }
  ...
  const moveColumn = async (columnId: string, direction: 'left' | 'right') => {
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite reorganizar colunas.');
      return;
    }
  ...
  const moveTask = async (taskId: string, direction: 'left' | 'right') => {
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite mover tarefas.');
      return;
    }
  ...
  const deleteTask = async (taskId: string) => {
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite excluir tarefas.');
      return;
    }
  ...
  const handleSaveTask = async (e: React.FormEvent) => {
    ...
    if (!canManageKanban) {
      setErrorMessage('Seu acesso neste workspace não permite criar ou editar tarefas.');
      return;
    }
  ...
  return (
    <div className="space-y-8 h-[calc(100vh-160px)] flex flex-col">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Button
            ...
            disabled={!canManageKanban || isLoadingPermissions}
          >
          ...
          <Button
            ...
            disabled={!canManageKanban || isLoadingPermissions || columns.length === 0}
          >
            <Plus className="h-4 w-4" />
            Adicionar Tarefa
          </Button>
      ...
      {!canManageKanban && !isLoadingPermissions && (
        <Card className="border-amber-200 bg-amber-50 p-4 text-amber-700">
          Você pode acompanhar o Kanban deste workspace, mas não pode criar, mover ou excluir itens.
        </Card>
      )}
            ...
            >
              <div className="flex items-center justify-between mb-4 px-2">
                ...
                <Dropdown
                  trigger={
                    <button
                      className="text-gray-400 hover:text-text-primary p-1 rounded hover:bg-gray-100"
                      disabled={!canManageKanban || isLoadingPermissions}
                    >
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <Dropdown
                          trigger={
                            <button
                              className="text-gray-300 group-hover:text-gray-400 p-1 rounded hover:bg-gray-100"
                              disabled={!canManageKanban || isLoadingPermissions}
                            >
                              <MoreHorizontal className="h-4 w-4" />
                            </button>
        ...
        <form onSubmit={(e) => void handleSaveTask(e)} className="space-y-4">
          ...
          <div className="flex justify-between pt-4">
            ...
            <div className="flex gap-3">
              ...
              <Button type="submit" isLoading={isSavingTask} disabled={!canManageKanban || isLoadingPermissions}>
