...
export const IdeasBank = () => {
  ...
  const handleSaveIdea = async (e: React.FormEvent) => {
    ...
    if (!canManageIdeas) {
      setErrorMessage('Seu acesso neste workspace não permite criar ou editar ideias.');
      return;
    }
  ...
  const handleDeleteIdea = async (id: string) => {
    if (!canManageIdeas) {
      setErrorMessage('Seu acesso neste workspace não permite excluir ideias.');
      return;
    }
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          disabled={!canManageIdeas || isLoadingPermissions}
        >
      ...
      {!canManageIdeas && !isLoadingPermissions ? (
        <Card className="border-amber-200 bg-amber-50 p-4 text-amber-700">
          Você pode consultar as ideias deste workspace, mas não pode criar, editar ou excluir.
        </Card>
      ) : null}
