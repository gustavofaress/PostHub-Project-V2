...
export const MobileTopBar = ({
  ...
  return (
    <header className="relative z-30 overflow-hidden md:hidden">
      ...
      <div
        className={cn(
          'relative mx-auto max-w-md pt-[calc(0.95rem+env(safe-area-inset-top))]',
          isCompact ? 'px-4 pb-3' : 'px-5 pb-5'
        )}
      >
        {isCompact ? (
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <p className="truncate text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-white/72">
                {subtitle || 'Workspace mobile'}
              </p>
              <h1 className="mt-1 truncate text-[2.05rem] font-semibold tracking-[-0.05em] text-white">
                {title}
              </h1>
              <button
                type="button"
                onClick={onOpenProfiles}
                className="mt-3 inline-flex max-w-full items-center gap-2 rounded-full border border-white/18 bg-white/14 px-3 py-2 text-left text-white/92 shadow-[0_14px_32px_rgba(15,23,42,0.12)] active:scale-[0.98]"
              ...
              >
                ...
                <ChevronDown className="h-4 w-4 shrink-0 text-white/78" />
              </button>
            </div>
            ...
            <button
              type="button"
              onClick={onOpenMenu}
              className="mt-1 flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white text-brand shadow-[0_12px_28px_rgba(15,23,42,0.16)] active:scale-[0.98]"
      ...
      <div
        className={cn(
          'pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-b from-transparent to-[#EEF6FB]',
          isCompact ? 'h-5' : 'h-8'
        )}
      />
    </header>
