...
import { ApprovalPost, ApprovalComment, loadApprovals, loadComments } from '../modules/approval/ApprovalModule';
...
export const PublicApprovalPage = () => {
  ...
  const handleStatusChange = async (newStatus: 'approved' | 'changes_requested' | 'rejected') => {
    ...
    try {
      ...
      if (comment.trim()) {
        ...
        const createdComment = await approvalService.addApprovalFeedback(newCommentData, { token });
  ...
  const handleSubmitFeedback = async () => {
    if (!post || !comment.trim() || !token) return;
    ...
    if (!url) {
      return (
        <div className={cn(className, 'bg-gray-800 flex items-center justify-center text-gray-500')}>
          {isLostVideo ? <Video className="h-8 w-8 opacity-50" /> : <Image as ImageIcon className="h-8 w-8 opacity-50" />}
        </div>
      );
    }

    const mediaElement = isVideo ? (
      <video src={url} className={className} controls autoPlay muted={false} loop playsInline>
        <p>Seu navegador não suporta vídeo HTML.</p>
      </video>
    ) : (
      <div className="relative h-full w-full">
        <img src={url} alt="Prévia" className={className} referrerPolicy="no-referrer" />
        {isLostVideo && (
        ...
        <div className="relative h-full w-full group">
          ...
          <div className="absolute inset-0 flex items-center justify-between p-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                setCarouselIndex((prev) => Math.min((post.mediaItems?.length || 1) - 1, prev + 1));
              }}
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="p-8">
            ...
            <div className="space-y-4 mb-8">
              <Button className={cn('w-full gap-3 h-14 text-lg', post.status === 'approved' ? 'bg-green-600' : '')} onClick={() => handleStatusChange('approved')}>
              ...
              {comments.length > 0 ? (
                comments.map((item) => (
                  <div key={item.id} className="flex gap-3">
                    <Avatar fallback={item.authorName.substring(0, 2).toUpperCase()} size="sm" />
